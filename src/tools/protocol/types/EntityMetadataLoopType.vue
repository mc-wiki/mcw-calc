<script setup lang="ts">
import { ref } from 'vue'
import { useI18n } from 'vue-i18n'
import { isActionKey } from '../constants.ts'
import TypeChoice from './TypeChoice.vue'

const props = defineProps<{ data: object; version: number }>()
const { t } = useI18n()

const errorState =
  !Array.isArray(props.data) || props.data[0] !== 'entity_metadata_loop' || props.data.length !== 2
const content = (props.data as any[])[1] as { type: string | object; endVal: number }

const showSubType = ref(false)
</script>
<template>
  <div class="complex-padding flex">
    <span v-if="errorState" class="error-state">{{ t('protocol.error.data') }}</span>
    <span v-else class="flex-1">
      {{ t('protocol.type.entity_metadata_loop', { endVal: content.endVal }) }}
    </span>
    <span
      v-if="!errorState"
      class="ml-2 cursor-pointer action-text"
      tabindex="0"
      @click="showSubType = !showSubType"
      @keyup="(e: KeyboardEvent) => isActionKey(e) && (showSubType = !showSubType)"
    >
      [{{ showSubType ? t('protocol.action.collapse') : t('protocol.action.expand') }}]
    </span>
  </div>
  <TypeChoice
    v-if="showSubType"
    :data="content.type"
    :version="version"
    type="div"
    class="sub-type"
  />
</template>
