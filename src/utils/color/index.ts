...
class Entries{
    ...
    */
    ...
    g_dyes(i: number, j: number){
        return ((new Array((this.dyes[(i*4 + j)*2] >>> 28) & 0xf)).fill(0).map((v,ii)=>
...
class MixU{
    ...
    update(i: number[]){
        ...
        // if we've mixed with the color too many times before
        // if(b !== NO_ENTRY && entries.mixc[b] >= MIX_LIMIT) return;
        // could be entries.s_mixc(entries.g_mixc(b) + 1) instead;
        this.entries.mixc[b]++;
...
export function colorToRecipeJava(
  ...
  let minSequence: number[][] = []
  let nextIndex = 0;
  while(nextIndex !== NO_ENTRY){
    // the 1 is for the least dyes used total in the recipe;
    nextIndex = entries.g_dyec(minIndex, 1);
    // grab the dye indices and add them to minSequence
    minSequence.push(entries.g_dyes(minIndex, 1));
  }
  
  return [minSequence, minDeltaE, separateRgb(minIndex)]
}
